...
| `M05` | Text rasterization and glyph layout | `formula tuning` | all three | Native consumes recovered glyph ids and no longer uses hidden char lookup. Exact static `Montserrat-BoldItalic.ttf` and `Point-Light.ttf` fixture resolution are locked. CoolType-shaped metric telemetry is implemented from TTF/CoolType evidence: glyph ids, hmtx advances, fixed16 advances, design-unit bboxes, scaled bboxes, sourceRect union, glyph-run/char/word/line indices, comp-space bbox/minmax/centers/baselines, metric source, and `cooltype_metric_verified` status. AE sourceRect-based text telemetry refs are imported for `TXT_010`/`TXT_020`/`TXT_030`/`TXT_040` and pass exactly for the covered cases. Static+dynamic BEE closes the cached fill route: `BEE_TextRenderNode` vtable `+0xd8` (`0x1805c0c20`) extracts `TXT_GridChar` stride `0x460`, gates fill/stroke, clips bounds, then calls `TXT_DrawChar` through BEE IAT to `TXT.dll+0x413d0`. Native has a concrete `txt_drawchar_boundary/v1` trace and uses the same boundary rows as raster input: glyph id, font identity, glyph/text matrices, fill/stroke gates/colors, clipped bounds, and bitmap bounds. The default coverage backend is now `ttf_outline_nonzero_supersample_v1`: TTF outlines, nonzero winding, fixed 4x supersampling, cached by font/glyph/size. The final render-time ARE path is now identified separately from the sourceRect outline player: `TXT_DrawChar -> TXTp_DrawChar3_ARE -> TXT_DrawOutlinePlayerARE vtable TXT.dll+0x6983e0`. Ghidra recovered the 8 bpc `PF_Pixel8` writer at `0x18003cb10`/`0x18003c980`, including AE integer alpha multiply and source-over shape; native covered-glyph pixels now use `recovered_txt_are_pf_pixel8_integer_source_over_v1`. Master gate stayed regression-free and text/collapse moved `TXT_010 14.088211 -> 14.070786`, `TXT_020 5.860491 -> 5.858233`, `TXT_030 6.803962 -> 6.502396`, `GPH_010 1.701600 -> 1.687255`; `TXT_040` stayed same bucket with tiny drift `3.727372 -> 3.730953`. | Lock exact CoolType/BIB `TXT_DrawChar` coverage rows: hinting/grid-fit, antialias/subpixel policy, clipped-bounds rounding, stroke/fill merge, semi-transparent fill temp-world/PF_TransferRect behavior, and literal `PFp_G_fcd_table` dump if 8 bpc unpremultiply needs byte-for-byte lock. Then move selector geometry from guarded placement only behind `TXT_010`/`TXT_020`/`TXT_040`/`GPH_010` no-regression gates. |
...
| `M07` | Character text animator position/scale/rotation/blur | `instrumented/testable` | `impulse_2nd` | Per-unit transforms, blur splat approximation, glyph-level unit rectangles, per-unit glyph refs, final matrix, opacity alpha scale, blur radius telemetry, and a P4 selector-composition helper that reports range weight, expression weight, final weight, text index/total, and local delayed time. | Wire the P4 selector-composition helper into render-core runtime, then add AE glyph animator goldens and tune per-glyph transform center, blur kernel, opacity composition, and selector weighting. |
| `M08` | Expression selector bounce | `implemented approximate` | `impulse_2nd` | Recognized generated `per_character_bounce` selector with deterministic native evaluator path. The evaluator now honors the generated pre-delay `else { value }` branch when `ctx.value` is supplied and keeps the old zero fallback when it is absent. | Expression selector Amount telemetry, AE bounce curve samples, and render-core integration of the explicit selector-composition helper. |
...
| `M11` | Glow | `formula tuning` | `template_4th` | Effect module, typed/numbered params, unit tests, effects conformance scaffold, time-aware params, straight-RGBA alpha-policy sidecar, threshold/blurred/scaled/final alpha stats, Frida-confirmed Glow radius route `IR_GaussianBlur(radius * 0.4)`, Ghidra-recovered ImageRenderer recursive Gaussian coefficients/zero-padding pass structure, Glow.aex `0006` operation table mapping (`1` none, `2` normal, `3` add, `6` screen), and float staging across horizontal+vertical recursive Gaussian passes before final RGBA8 quantization. `EFF_020` primary visible mean is now `1.629317`; `EFF_070` remains low-error at `0.353011`; master gate has `regression=0`. | Remaining work is effect-local Glow `IR_CompositeWithBlendMode` blend/premult worker semantics, intensity/color-loop controls, and unsupported operation modes. Do not retune Gaussian constants without new isolated probe ownership. |
...
| `M14` | Turbulent Displace | `reverse implemented (FracAll/Frac1D table-state subset)` | `scenes_3rd` | Frida/Ghidra recovered the CPU `PF_CMD_RENDER` route (`cmd11 -> df30 -> 3410/7de0/d070/e340/d120`), AE LCG table build, bspline interpolation, octave/fractal constants, `FracAll` field pair, recovered `Frac1D` H/V lookup buffers for public displacement types `7..9`, fixed16 displacement scale, source-UV sign/swap rules, default offset-center, source-space-before-transform routing for footage Turbulent-only stacks, and `FUN_180007840` pinning fade. Added Frida table, pin-radius/corner-fade, and Frac1D branch regressions. `EFF_060` visible mean is now `0.166828` with alpha `0` and max diff `1..2`; composed `STK_030` is low-error at `0.332818`; Geometry2 gates remain stable. | Remaining M14 work: recover/lock antialiasing footprint branch, resize-layer edge helpers, cycle-evolution/nonzero-seed probes, and then rerun full master gate. Do not continue by metric-only fitting. |
...
```text
target/ae_agents/p0_master_gate_integrated_p1_p3_p4_20260508/dashboard.md
```

Summary:

```text
cases=19 accepted=2 approximate=16 tuning=1 regressions=0 missing=0
```
...
| `scenes_3rd` | `approximate` | `TMP_020` is accepted; isolated `EFF_060` is accepted, and `STK_030` is low-error approximate at `0.332818` after Turbulent bucket-world routing plus the Frac1D branch-coverage pass. |
...
The runtime raster path now uses recovered glyph ids, an explicit
`txt_drawchar_boundary/v1` draw plan, and a default TTF-outline coverage
backend. The plan records the same boundary we recovered from
`BEE_TextRenderNode -> TXT_DrawChar`: glyph id, font identity, glyph/text
matrices, fill/stroke enable flags and colors, clipped bitmap bounds, coverage
backend metadata, and output semantics. The render-time
`TXT_DrawOutlinePlayerARE` pixel writer has also been separated from the
sourceRect outline player; native covered pixels now use the recovered AE
integer `PF_Pixel8` source-over path. A naive full CoolType behavior switch
improved text-only cases but regressed `GPH_010`; the current outline coverage
backend is narrower and gate-safe. See
`docs/phase_reports/P2_COOLTYPE_METRIC_LAYOUT_GUARD_20260507.md` and
`docs/phase_reports/P2_TEXT_RASTER_COVERAGE_20260507.md` and
`docs/phase_reports/P2_TXT_DRAWCHAR_NATIVE_BOUNDARY_20260507.md` and
`docs/phase_reports/P2_TTF_OUTLINE_TEXT_COVERAGE_20260507.md` and
