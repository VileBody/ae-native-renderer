...
| `M12` | Geometry2 | `instrumented/testable` | `scenes_3rd` | Adjustment effect module with transform-like params, time-varying scalar support, matrix/sample debug data, adjustment-stack debug sidecars, isolated coordinate-field edge probe, integer pixel-center evidence, partial-footprint transparent bilinear edge policy, Frida CPU-path trace (`Transform.aex+0x5f30`), wrapper ABI trace (`Transform.aex+0x5b20`), and confirmed `0012` Sampling mapping (`1` bilinear, `2` bicubic). | Tune the `0012 = 2` cubic kernel, edge footprint, and rounding against isolated coordinate-field evidence; residual `EFF_040` error is sampler-owned until that branch is fitted, so keep matrix math frozen before stack-level changes. |
| `M13` | Minimax | `instrumented/testable` | `scenes_3rd` | Effect module with AE operation/channel/direction enum surface, time-aware radius, AE discriminator probe for fractional radius/direction/channel/edge behavior, and native `Don't Shrink Edges` edge policy. | Commit durable goldens if needed, add deep internal-alpha edge probe, then GPU/CPU path parity only if template evidence shows divergence. |
