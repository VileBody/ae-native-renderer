...
| `M16` | Adjustment layer pipeline and effect-stack order | `instrumented/testable` | `scenes_3rd` | Adjustment layers apply known effects to accumulated canvas; per-effect input/output hashes, bucket/live param times, and Geometry2/Minimax/Turbulent debug checkpoints are logged for adjustment stacks. `STK_031` now isolates a Geometry2-on-adjustment residual: primary visible RGB mean `8.511274` while isolated `EFF_041` is `0.043613`. | Isolate AE adjustment/canvas contract before changing global ordering: background RGB/alpha substrate, effect input canvas, and whether transform-like adjustment effects preserve or replace lower-stack transparent regions. |
