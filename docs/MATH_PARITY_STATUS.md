...
The runtime raster path now uses recovered glyph ids, but coverage/fill is
still fontdue-based and placement remains guarded on the pre-existing bitmap
geometry. A naive full CoolType behavior switch improved text-only cases but
regressed `GPH_010`; see
