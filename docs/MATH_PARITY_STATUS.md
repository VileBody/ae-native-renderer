...
| `M05` | Text rasterization and glyph layout | `formula tuning` | all three | Fontdue remains the stable coverage rasterizer, but runtime now consumes recovered glyph ids via `rasterize_indexed(glyph_id, size)` instead of char lookup. Exact static `Montserrat-BoldItalic.ttf` and `Point-Light.ttf` fixture resolution are locked. CoolType-shaped metric telemetry is implemented from TTF/CoolType evidence: glyph ids, hmtx advances, fixed16 advances, design-unit bboxes, scaled bboxes, sourceRect union, glyph-run/char/word/line indices, comp-space bbox/minmax/centers/baselines, metric source, and `cooltype_metric_verified` status. AE sourceRect-based text telemetry refs are imported for `TXT_010`/`TXT_020`/`TXT_030`/`TXT_040` and pass exactly for the covered cases. Frida `RAS_010` confirms `TXT_PlayCharOutlines` emits glyph id `331` for Montserrat `W`, matching native. Static+dynamic BEE closes the cached fill route: `BEE_TextRenderNode` vtable `+0xd8` (`0x1805c0c20`) extracts `TXT_GridChar` stride `0x460`, gates fill/stroke, clips bounds, then calls `TXT_DrawChar` through BEE IAT to `TXT.dll+0x413d0`. Native now has a concrete `txt_drawchar_boundary/v1` trace and uses the same boundary rows as raster input: glyph id, font identity, glyph/text matrices, fill/stroke gates/colors, clipped bounds, bitmap bounds, and explicit straight-RGBA/PF_World semantics note. Focused text passports report `txt_drawchar_boundary_ready=true` for `TXT_010`; `26` glyph rows map to `26` boundary rows and `23` drawable rows. | Replace the remaining `fontdue_rasterize_indexed` coverage backend with CoolType-compatible `TXT_DrawChar` coverage/fill semantics. After that, move selector geometry from guarded fontdue placement only behind `TXT_010`/`TXT_020`/`TXT_040`/`GPH_010` no-regression gates. |
...
The runtime raster path now uses recovered glyph ids and an explicit
`txt_drawchar_boundary/v1` draw plan. The plan records the same boundary we
recovered from `BEE_TextRenderNode -> TXT_DrawChar`: glyph id, font identity,
glyph/text matrices, fill/stroke enable flags and colors, clipped bitmap
bounds, and output semantics. The coverage/fill backend is still fontdue-based
and placement remains guarded on the pre-existing bitmap geometry. A naive full
CoolType behavior switch improved text-only cases but regressed `GPH_010`; see
