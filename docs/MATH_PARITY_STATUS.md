...
| `M12` | Geometry2 | `instrumented/testable` | `scenes_3rd` | Adjustment effect module with transform-like params, time-varying scalar support, matrix/sample debug data, adjustment-stack debug sidecars, isolated coordinate-field edge probe, integer pixel-center evidence, partial-footprint transparent bilinear edge policy, Frida CPU-path trace (`Transform.aex+0x5f30`), wrapper ABI trace (`Transform.aex+0x5b20`), confirmed `0012` Sampling mapping (`1` bilinear, `2` bicubic), and fitted `0012=2` Keys cubic kernel (`a=-0.7`) from AE85 Frida-gated probes. | Add alpha-specific sampler cases and a composed `0012=2` scene check; matrix math remains frozen unless isolated opaque sampler evidence contradicts it. |
