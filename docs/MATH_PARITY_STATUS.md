...
| `M11` | Glow | `formula tuning` | `template_4th` | Effect module, typed/numbered params, unit tests, effects conformance scaffold, time-aware params, straight-RGBA alpha-policy sidecar, threshold/blurred/scaled/final alpha stats, Frida-confirmed Glow radius route `IR_GaussianBlur(radius * 0.4)`, Ghidra-recovered ImageRenderer recursive Gaussian coefficients/zero-padding pass structure, and Glow.aex `0006` operation table mapping (`1` none, `2` normal, `3` add, `6` screen). `EFF_020` primary visible mean is now `1.628605`; `EFF_070` remains low-error at `0.352847`; master gate has `regression=0`. | Remaining work is effect-local Glow blend/premult worker semantics for non-add modes, intensity/color-loop controls, and tighter Gaussian conversion/quantization if new isolated probes show residual ownership. |
