...
ADBE Glo2              ImageRenderer recursive Gaussian subset + Glow operation/composite subset
...
| `ADBE Glo2` | `based_on`, `threshold`, `radius`, `intensity`, `composite_original`, `operation` | `0001` glow based on (`1` alpha channel, `2` color channels; absent uses the AE default `2` color channels), `0002` threshold, `0003` radius, `0004` intensity, `0005` composite original (`1` behind, `2` on top), `0006` glow operation (`1` none, `2` normal, `3` add, `6` screen; other modes currently fall back to add) |
...
`ir_gaussian_radius = glow_radius * 0.4`. Native now runs the Glow-local
ImageRenderer recursive Gaussian subset recovered from `IR_GaussianBlur`: causal
and anti-causal passes, zero padding, and horizontal+vertical filtering. Glow
Based On defaults to color/luma channels, and `0005`/`0006` use the recovered
Glow.aex operation/composite mapping for none/normal/add/screen. Box Blur and
Drop Shadow keep their separate GF/alpha-blur paths. Remaining Glow residuals
belong to effect-local ImageRenderer blend/premult details and unsupported Glow
Operation modes, not to the global M19 normal-composite contract.
