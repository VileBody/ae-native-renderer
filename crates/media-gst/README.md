# media-gst

Media adapter crate for probe/decode/encode boundaries.

Current state:

- `doctor` checks GStreamer availability through `gst-inspect-1.0`.
- active probe paths use FFmpeg/ffprobe as bootstrap tooling.
- active frame extraction supports GStreamer appsink and a persistent FFmpeg
  rawvideo pipe fallback.
- active MP4 output supports GStreamer appsrc and FFmpeg CLI fallback.
- `VideoSource` and `VideoSink` are the media boundaries used by the CLI today.
- the CLI adds a per-source frame cache and bounds the number of open decoders.
- media tuning knobs: `AE_RENDER_MAX_OPEN_DECODERS`,
  `AE_RENDER_MEDIA_FRAME_CACHE`, `AE_RENDER_MAX_SEQUENTIAL_DECODE_GAP`,
  `AE_RENDER_MEDIA_BACKEND`, and `AE_RENDER_MUX_BACKEND`.

Target state:

- production hardening of GStreamer `appsink`/`appsrc` pipelines.
- FFmpeg remains available as fallback/debug tooling, not as renderer core.

See `docs/ARCHITECTURE.md` for the full media I/O split.
